...
def _vsed_debug(
    ...
    ## end 側をスライド
    end2 = len(zcs)
    for i in range(_my_round(end1_s / hop_length_s), len(zcs), 1):
        if i < len(data) and zcs_threshold <= zcs[i]:
            zcs_temp = zcs[i: i+int(zcs_margin/hop_length)]
    ...
    temporal_positions = np.linspace(0, len(zcs) * hop_length_s, len(zcs))
    return start1_s, end1_s, start2_s, end2_s, \
        temporal_positions, x_rms, zcs, zcs_threshold
