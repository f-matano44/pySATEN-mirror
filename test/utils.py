...
def gen_noise_signal(
        x: np.array, fs: int, snr: int, is_white: bool,
        rand: np.random.default_rng, ans_s_sec: float, ans_e_sec: float
    ):
    speech_start_idx: int = int(ans_s_sec * fs)
    speech_end_idx:   int = int(ans_e_sec * fs)
    # generate noise (white or pink)
    noise = rand.uniform(low=-1.0, high=1.0, size=len(x)) \
        if is_white else _gen_pink_noise(len(x), fs, rand)
...
def _gen_pink_noise(
        length: int, fs: int, rand: np.random.default_rng
    ) -> np.array:
    length2 = length + 1000
